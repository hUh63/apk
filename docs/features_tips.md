...
- 悬浮球：==液态玻璃风格正圆==（径向渐变 + 顶部高光 + 白色波纹图案，无描边），==先在「悬浮球权限」入口授权==（未授权时启用开关禁用）
- 点击悬浮球弹出==贴球配置面板==：`设置`（打开应用）/ `换颜色` / `调透明度` / `贴边开关` / `隐藏悬浮球`；面板在球的内侧弹出，配置==实时写回偏好设置==，与设置页状态同步
- 3 秒无操作自动贴边（可关），支持拖动调整位置
- ==看不到悬浮球？==启动失败会弹窗显示具体原因；按提示检查系统「显示悬浮窗」与厂商「后台弹出界面」权限（MIUI/HyperOS/ColorOS 等有独立开关）
- Shizuku：==确保 Shizuku 应用处于运行状态==再点「请求 Shizuku 授权」（==未授权时按钮才显示==，已授权自动隐藏），授权弹窗在应用内弹出，允许即完成；状态栏显示 已授权/未授权/未连接 三态
