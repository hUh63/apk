...
- 悬浮球：==先在「悬浮球权限」入口授权==（实时显示状态，未授权时启用开关禁用）；开启后桌面显示 MCP 状态球（==绿色描边=MCP 运行中==），点击弹快捷面板（打开应用/刷新状态），3 秒无操作自动贴边（可关，支持拖动）
- 样式自定义：预置 6 色 + 取色器 + 透明度滑杆 + 实时预览
- ==看不到悬浮球？==启动失败会弹窗显示具体原因；按提示检查系统「显示悬浮窗」与厂商「后台弹出界面」权限（MIUI/HyperOS/ColorOS 等有独立开关）
- Shizuku：==确保 Shizuku 应用处于运行状态==再点「请求 Shizuku 授权」，授权弹窗在应用内弹出，允许即完成；ProxyPin 会出现在 Shizuku 的应用管理列表中
...
- 实现位置：`lib/network/util/idn.dart`（RFC 3492 编码），连接层 `network.dart` 统一清洗后再建连
