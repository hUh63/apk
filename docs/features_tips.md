...
- 悬浮球：==76dp 液态玻璃正圆==（径向渐变 + 顶部高光 + 白色波纹图案，无描边，可拖动/3 秒自动贴边），==先在「悬浮球权限」入口授权==（未授权时启用开关禁用）
- 点击悬浮球弹出==球旁配置面板==（16dp 间距不重叠、垂直对齐球心）：`设置`（打开应用）/ `换颜色` / `调透明度` / `贴边开关` / `隐藏悬浮球`；配置==实时写回偏好设置==，与设置页状态同步；设置页「自定义悬浮球」实时预览与真实球一致
